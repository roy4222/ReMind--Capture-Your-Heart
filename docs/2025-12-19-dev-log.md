...
| Discord Send 節點 | 使用 `$json.output` 而非 `$node["節點名"].json.output` |

---

### 問題 6：Discord Send - Cannot send an empty message
...
**原因**：使用 `$node["回應區"].json.output` 引用資料時，表達式評估可能失敗導致 embed 內容為空。

**解決方案**：
改用 `$json.output` 引用（因為有 main connection，資料會自動傳入）：

```json
// 修改前
"content": "={{ $node[\"回應區\"].json.output || '處理中...' }}"

// 修改後
"content": "={{ $json.output || '處理中...' }}"
```

**社群最佳實踐**：當節點有 main connection 時，優先使用 `$json` 引用資料，而非顯式的節點名稱引用。
...
- [x] 完整對話流程測試 - **通過**
- [x] 事實提取存入 Qdrant 測試 - **通過**（NONE 被正確過濾）
- [ ] 多用戶記憶隔離測試 - 待測試

**所有節點執行狀態**：
- Discord Trigger: Success (4.175s)
- 回應區: Success (2.322s, 735 Tokens)
- 記憶區: Success (324ms, 147 Tokens)
- 過濾無效事實: Success (1ms)
